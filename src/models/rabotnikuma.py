...
# author Hiroshi Yoshihara repo: https://github.com/analokmaus/kaggle-g2net-public
...
class AdaptiveGeM(nn.Module):
    ...
    def forward(self, x):
        return F.adaptive_avg_pool2d(x.clamp(min=self.eps).pow(self.p), self.size).pow(
            1.0 / self.p
        )
        ...
        for spectrogram, is_trainable in zip(self.spectrograms, self.is_wavegram):
            if is_trainable:  # Use CNN as spectrogram
                ...
                with torch.cuda.amp.autocast(
                    enabled=False
                ):  # MelSpectrogram causes NaN with AMP
                    spec = spectrogram(
                        s.view(bs * ch, w)
                    )  # spec: (batch size * wave channel, freq, time)
                _, f, t = spec.shape
...
class SpectroCNN(nn.Module):
    def __init__(
        ...
        self.cnn = timm.create_model(
            model_name, pretrained=pretrained, num_classes=num_classes, **timm_params
        )
        ...
        if custom_classifier != "none" or custom_attention != "none":
            ...
            if custom_classifier == "avg":
                ...
                global_pool = nn.Sequential(GeM(p=3, eps=1e-4), ManifoldMixup())
                self.mixup_mode = "manifold"
            else:
                raise ValueError(f"Unsupported classifier type: {custom_classifier}")

            if (
                model_type[-11:] == "Transformer" or model_type == "XCiT"
            ):  # transformer models
    ...
    def forward(
        self, s, lam=None, idx=None
    ):  # s: (batch size, wave channel, length of wave)
        bs, ch, w = s.shape
...
class MultiInstanceSCNN(nn.Module):
    def __init__(
        ...
        self.cnn = nn.Sequential(
            preprocess, self.cnn, attention, global_pool, Flatten()
        )
    ...
    def forward(self, s):
        ...
        if self.n_patch > 1:
            spec = self._make_patches(
                spec, self.n_patch
            )  # spec: (bs, patch, ch, freq, time)
        else:
            spec = spec.unsqueeze(1)
        _, _, ch, f, t = spec.shape
        spec = spec.view(
            bs * self.n_patch, ch, f, t
        )  # spec: (bs*patch, ch, freq, time)
